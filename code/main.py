...
while True:
    ...
    if paused:
        ...
        pause_message = pygame.image.load("images/Pause/MensagemPausaTopo.png")
        pause_message_rect = pause_message.get_rect(top = 25, left = 425)

        menu = pygame.image.load("images/Pause/Menu.png")
        menu_rect = menu.get_rect(top = 300, left = 425)

        first_option = pygame.Rect(425, 300, 227.5, 62.5)
        second_option = pygame.Rect(425, 387.5, 227.5, 62.5)
        third_option = pygame.Rect(425, 475, 227.5, 62.5)
        ...
        if first_option.collidepoint(mouse_pos):
            highlight_area(first_option, screen)
        if second_option.collidepoint(mouse_pos):
            highlight_area(second_option, screen)
        if third_option.collidepoint(mouse_pos):
            highlight_area(third_option, screen)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if first_option.collidepoint(event.pos):
                    paused = not paused
                if second_option.collidepoint(event.pos):
                    paused = not paused
                if third_option.collidepoint(event.pos):
                    paused = not paused
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE and currentScreen not in ["TitleScreen", "ChapterSelection", "PreGameTransition"]:
                    paused = not paused
